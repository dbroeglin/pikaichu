class TaikaiEvent < ApplicationRecord
  ...
  def self.rectification(taikai:, user:, result:, previous_status:, previous_value: nil)
    ...
    if taikai.scoring_enteki?
      ...
      message = I18n.t 'templates.taikai_event.rectification.enteki', user: user.display_name, taikai: taikai.name,
                                                                      round: result.round,
                                                                      index: result.index,
      ...
      message = I18n.t 'templates.taikai_event.rectification.kinteki', user: user.display_name, taikai: taikai.name,
                                                                       round: result.round,
                                                                       index: result.index,
                                                                       status: Result.human_enum_value(:status, result.status),
                                                                       previous_status: Result.human_enum_value(:status, previous_status),
